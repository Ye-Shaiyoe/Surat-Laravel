    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body {
            ...
            padding: 32px 16px;
            position: relative;
            /* Agar halaman tetap bisa di-scroll saat konten melebihi tinggi viewport (terutama di HP). */
            overflow-x: hidden;
            overflow-y: auto;
...
<body>
    ...
    <div class="glass-card">
        ...
        <div class="card-sub">Daftarkan akun baru Anda</div>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="field-wrap">
                ...
                <input class="field-input" id="name" type="text" name="name" value="{{ old('name') }}" placeholder="Nama lengkap Anda" required autofocus autocomplete="name">
                @error('name')<p class="error-msg">{{ $message }}</p>@enderror
            </div>

            <div class="field-wrap">
                <label class="field-label" for="email">Email</label>
                <input class="field-input" id="email" type="email" name="email" value="{{ old('email') }}" placeholder="nama@instansi.go.id" required autocomplete="username">
                @error('email')<p class="error-msg">{{ $message }}</p>@enderror
            </div>

            <div class="field-wrap">
                <label class="field-label" for="password">Kata Sandi</label>
                <input class="field-input" id="password" type="password" name="password" placeholder="••••••••" required autocomplete="new-password">
                @error('password')<p class="error-msg">{{ $message }}</p>@enderror
            </div>

            <div class="field-wrap">
                <label class="field-label" for="password_confirmation">Konfirmasi Kata Sandi</label>
                <input class="field-input" id="password_confirmation" type="password" name="password_confirmation" placeholder="••••••••" required autocomplete="new-password">
        ...
        <a href="{{ route('login') }}" class="login-link">Sudah punya akun? Masuk di sini</a>
    </div>
